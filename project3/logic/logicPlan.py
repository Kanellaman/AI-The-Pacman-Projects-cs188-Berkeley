...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """ Used the Theorem from the slides of chapter 8 """
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for x, y in combs:
        if x != y:
            ls.append(disjoin(x, y))
    """ The conjoin of the list that we return it is of form (~X | ~Y) & (~X | ~Z) & (~Y | ~Z) & etc ..... 
    That way we emsure that if 2 (or more) literals of the list "literals" are True then we get False (~True | ~True) >> False.
    If at most 1 is True then we can see that is True """
    return conjoin(ls)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def exactlyOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    """ atLeastOne + atMostOne => exactlyOne expression in the list is true """
